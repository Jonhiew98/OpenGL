...
int Init(void)
{
	...
	loadTexture("../media/sohai.bmp", GtextureID[0]);
...
void UpdateCamera(void)
{
	...
	gViewMatrix = Matrix4::translate(Vector3(0.0f, 0.0f, -distance)) * Matrix4::rotate(yaw,Vector3(1.0f, 0.0f, 0.0f))*Matrix4::rotate(pitch,Vector3(0.0f, 1.0f, 0.0f));
}
...
void Draw(void)
{
	...
	GLint spectrumLeft = glGetUniformLocation(GprogramID, "SpectrumLeft");
	if (spectrumLeft != -1)
	{
		glUniform1f(spectrumLeft, m_spectrumLeft[0]);
	}

	GLint spectrumRight = glGetUniformLocation(GprogramID, "SpectrumRight");
	if (spectrumRight != -1)
	{
		glUniform1f(spectrumRight, m_spectrumRight[0]);
	}
   ...
   glDrawArrays(GL_TRIANGLES, 0, 3333);
